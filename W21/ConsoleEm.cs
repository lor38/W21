    ...
    public class ConsoleEm
    {
        ...
        public void Run()
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine(" 6. Wyszukaj pracownika");
                ...
                switch (option)
                {
                    ...
                    case "5":
                        ...
                        break;
                    case "6":
                        var found = FindEmployeeByName();
                        if (found != null)
                        {
                            ManageFoundEmployee(found);
                        }
                        break;
                    case "7":
                        Console.ForegroundColor = ConsoleColor.Cyan;
        ...
        static void AddEmployee()
        {
            ...
            if (typeOption == "1")
                employee = new EmployeeInFile(firstNameInput, lastNameInput);
            else if (typeOption == "2")
                employee = new SupervisorInFile(firstNameInput, lastNameInput);
            else
            ...
            while (true)
            {
                ...
                if (gradeInput.Equals("q", StringComparison.CurrentCultureIgnoreCase)) break;
                try
                {
        ...
        static void AddGradeToExistingEmployee()
        {
            ...
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine($"\n Wpisz oceny dla {employee.Name} {employee.Surname} (q aby zakończyć):");
            Console.ResetColor();
            while (true)
            {
                Console.ForegroundColor = ConsoleColor.Magenta;
        ...
        private IEmployee? FindEmployeeByName()
        {
            ...
            Console.Write("\n  Wpisz fragment imienia lub nazwiska: ");
            string? query = Console.ReadLine()?.Trim().ToLower();
            var matches = employees
                .Select((e, i) => new { Index = i, Employee = e })
                .Where(x =>
                    x.Employee.Name.ToLower().Contains(query ?? "") ||
                    x.Employee.Surname.ToLower().Contains(query ?? ""))
                .ToList();
            if (matches.Count == 0)
            {
                Console.ForegroundColor = ConsoleColor.Red;
            ...
            {
                Console.WriteLine($" {match.Index}. {match.Employee.Name} {match.Employee.Surname}");
            }
            Console.ResetColor();
            Console.Write("\n  Podaj indeks pracownika: ");
            if (int.TryParse(Console.ReadLine(), out int selectedIndex) &&
                matches.Any(m => m.Index == selectedIndex))
            {
        ...
        private void ManageFoundEmployee(IEmployee employee)
        {
            while (true)
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine($"\n  Co chcesz zrobić z {employee.Name} {employee.Surname}?");
                ...
                switch (choice)
                {
                    ...
                    case "4":
                        ...
                        DisplayStatistics(employee, stats, ConsoleColor.Green);
                        Pause();
                        break;
                    case "5":
                        return;
